...
- 已补充 `live_runs` 的服务端权威预检、发布版本校验、幂等创建、运行记录快照和停止请求；`browser_ingest` 会返回不可预测的流名和同源 SRS WHIP 入口。
- 已补充过渡期浏览器最终画面链路：前端把模板、数字人 Canvas、文本和图片合成为竖屏媒体流，混入数字人音频后通过 WHIP 进入 SRS；媒体 supervisor 等待源流上线后为每个目标独立转推，并支持心跳、浏览器断流宽限、目标指数退避重试和停播回收。显式启用的 `test_pattern` 继续用于无浏览器链路联调。
- 尚未完成独立服务端媒体 worker、租户/RBAC、审计日志、Vault/KMS 托管密钥、OAuth callback 和互动权限；浏览器合成是打通真实 RTMP 的过渡方案，长期生产形态仍按第 6 阶段迁移到服务端合成。
