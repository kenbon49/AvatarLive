...
class MuseTalkEngine:
    ...
    def _load_models(self) -> None:
        ...
        log.info("loading MuseTalk 1.5 models from %s", self.root)
        ...
        self.unet.model = self.unet.model.to(device=self.device, dtype=torch.float16).eval()
        ...
        log.info("MuseTalk 1.5 model load complete")
    ...
    def render_batch(self, audio_features: Any, frames: Sequence[Any]) -> NDArray[np.uint8]:
        ...
        with self._lock, self.torch.inference_mode():
            ...
            faces = self.vae.decode_latents(predictions)

        output = [
            self.blend_face(item.frame, face, item.face_box, item.blend_mask)
            for item, face in zip(items[:actual_count], faces[:actual_count])
        ]
        return np.stack(output).astype(np.uint8, copy=False)

    def warm_up(self, frames: Sequence[Any], iterations: int = 2) -> float:
        """Warm the fixed UNet/VAE batch shape and return elapsed seconds."""
        ...
        log.info("MuseTalk warm-up complete in %.3fs", elapsed)
        return elapsed
