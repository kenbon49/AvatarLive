...
| `model-en-mixed.bin` | HeadAudio `@main/dist/` | 预训练 viseme 模型(英语 MFCC+Gaussian,14KB),不入库 |
| `avatars/brunette.glb` | TalkingHead `@1.7/avatars/` | Ready Player Me 示例形象(**CC BY-NC 4.0,非商用**) |

## 📦 形象 GLB / HeadAudio 模型不入库

`avatars/*.glb` 单个 5–14MB，`model-en-mixed.bin` 是预训练口型模型，均已加进 `.gitignore` 不随仓库分发。本机跑 `/app/avatar-web` 前需把这些文件放回对应路径：
...
- `model-en-mixed.bin` — HeadAudio [`@main/dist/`](https://github.com/met4citizen/HeadAudio/tree/main/dist)
